...
import math
import os
os.environ["TORCHINDUCTOR_CACHE_DIR"] = "/workspace/inductor_cache"
...
ind_config.compile_threads = 128
...
def train(args):
    ...
    # torch.compile block ? all inside one if
    if getattr(args, 'dtorch_compile', False):
        compile_kwargs = dict(
        ...
        mmdit = torch.compile(mmdit, **compile_kwargs)

        if clip_l is not None:
            logger.info("Applying torch.compile to CLIP-L...")
            clip_l = torch.compile(clip_l, **compile_kwargs)

        if clip_g is not None:
            logger.info("Applying torch.compile to CLIP-G...")
            clip_g = torch.compile(clip_g, **compile_kwargs)

        if t5xxl is not None:
            logger.info("Applying torch.compile to T5XXL (this may take 10-20 minutes)...")
            t5xxl = torch.compile(t5xxl, **compile_kwargs)

        if vae is not None:
            logger.info("Applying torch.compile to VAE...")
            vae = torch.compile(vae, **compile_kwargs)
    ...
    if args.blockwise_fused_optimizers:
        ...
        grouped_params.extend(params_to_optimize[1:])
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # stepwise save ? DeepSpeed-safe
                if args.save_every_n_steps is not None and global_step % args.save_every_n_steps == 0:
                    accelerator.wait_for_everyone()

                    # Temporarily disable save_state inside model save (it's collective with DeepSpeed)
                    _save_state = args.save_state
                    if args.deepspeed:
                    ...
                    # Restore and do collective save_state with all ranks
                    args.save_state = _save_state
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dtorch_compile",
        action="store_true",
        help="Apply torch.compile to all models (MMDiT, CLIP-L, CLIP-G, T5XXL, VAE)",
    )
